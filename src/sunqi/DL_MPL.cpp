...
void Convert_to_Exact_Cover_Without_Intermediate(DancingLink & dl, int & row_numbers, int & col_numbers, std::string infilename,
	bool Whether_BFS, int & vertex_numbers, int & edge_numbers, int & mask_numbers, 
	std::vector<int> & MPLD_search_vector ) {

	std::vector<std::list<Edge_Simple>> edge_list = Read_Graph_File(infilename, vertex_numbers, edge_numbers);
	...
	for (int it = 1; it < edge_list.size(); ++it)
	{
		for (int i = 1; i <= mask_numbers; ++i)
		{
			for (auto j = edge_list[it].begin(); j != edge_list[it].end(); ++j)
			{
				Cell_Insert(dl, (it - 1)*mask_numbers + i, vertex_numbers + (j->No - 1)*mask_numbers + i);
			}
		}
	}
	...
	for (int i = 0; i < edge_numbers; ++i)
	{
		for (int j = 1; j <= mask_numbers; ++j)
		{
			Cell_Insert(dl, vertex_numbers * mask_numbers + 1, vertex_numbers + i * mask_numbers + j);
		}
	}
	return;
}
...
{
	std::vector<int> Final_Color;
	Final_Color.resize(vertex_numbers + 1);
	...
	for (int i = 1; i < Final_Color.size(); ++i)
	{
		fileout << "Vertex " << i  << " \t : " << Final_Color[i] << std::endl;
	}
	/*
...
void MPLD_Solver(std::string Graph_Filename, std::string Exact_Cover_Filename, bool whether_BFS, int mask_numbers)
{
	...
	Convert_to_Exact_Cover_Without_Intermediate(dl, row_numbers, col_numbers, Graph_Filename, whether_BFS, vertex_numbers, edge_numbers, mask_numbers, MPLD_search_vector);
