...
void LayoutDBPolygon::generate_rect_vectors(){
    ...
    for (std::vector<rectangle_data<coordinate_type> >::const_iterator it = vTmpRectangle.begin(), ite = vTmpRectangle.end(); it != ite; ++it)
    {
        ...
        //simply skip VDD TODO:it should be changed to fit VDD. 29.09.2019 Wei
        ...
        //NOTE: here layer information is lost due to that we generate pPattern when all of the polys are read
        pPattern->layer(1);
        //ALSO, color information is lost so that it does not support pre-color
        pPattern->color(-1);
...
void LayoutDBPolygon::initialize_data()
{
    ...
    // duplicates are removed with following function1
    //Only  used for placement benchmark
    generate_rect_vectors();
